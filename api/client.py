...
from utils.config_reader import get_environment_config, get_api_key
from utils.logger import get_logger
from pathlib import Path
from utils.assertions import assert_status_code

# Optional Allure support
try:
    import allure
...
class APIClient:
        ...
        for attempt in range(self.retry_count + 1):
            try:
                ...
                if response.status_code >= 500:
                    logger.warning(
                        f"Server error {response.status_code}, retrying ({attempt + 1})..."
                    )
    ...
    def _log_response(self, response: Response) -> None:
        ...
        logger.info(
            f"Response [{response.status_code}] | URL: {response.url} | Body: {body_preview}"
        )
    ...
    def validate_schema(self, response: Response, schema_file: str) -> None:
        ...
        try:
            # ✅ Flexible validation: validate only "data" block
            if (
